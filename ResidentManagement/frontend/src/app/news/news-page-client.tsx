    ...
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:py-12">
      <header className="flex flex-col gap-4 pb-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-primary/80">Bản tin Chính phủ</p>
            <h1 className="text-3xl font-heading font-semibold text-slate-900 dark:text-white md:text-4xl">
          ...
          <button
            ...
            className="hidden items-center gap-2 rounded-xl border border-primary/40 bg-primary/10 px-4 py-2 text-sm font-semibold text-primary transition hover:bg-primary hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-primary/50 dark:bg-primary/20 dark:text-white/90 dark:hover:text-white lg:flex"
          >
            <span>Đồng bộ</span>
      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-slate-200/60 bg-white/70 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/70 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex w-full items-center gap-3 md:w-80">
          <div className="relative w-full">
            <input
              ...
              className="w-full rounded-xl border border-slate-300 bg-white/90 px-4 py-2 text-sm text-slate-700 shadow-sm transition focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <button
            type="button"
            onClick={handleRefresh}
            className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-600 transition hover:bg-primary hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-primary/80"
          >
            ↻
          </button>
        </div>
      ...
      {error && (
        <div className="mt-4 rounded-2xl border border-amber-300 bg-amber-50/70 p-4 text-sm text-amber-900 shadow-sm dark:border-amber-500/60 dark:bg-amber-500/10 dark:text-amber-100">
          {error}
        </div>
      )}
