...
const Header = () => {
  ...
  return (
    <div className={`sticky top-0 z-[100] w-full flex justify-center transition-all duration-500 ease-in-out ${isScrolled ? 'pt-4' : 'pt-0'}`}>
      <header
        className={`text-white transition-all duration-500 ease-in-out origin-top
          ${isScrolled
            ? 'w-[95%] max-w-6xl rounded-2xl bg-gradient-to-r from-red-800/95 via-red-950/95 to-red-800/95 backdrop-blur-md shadow-xl'
            : 'w-full max-w-none rounded-none bg-gradient-to-r from-red-800 via-red-950 to-red-800 shadow-md'
          }`}
      >
        <div className={`container mx-auto flex flex-wrap items-center justify-between gap-3 px-4 transition-all duration-500 ease-in-out ${isScrolled ? 'py-2' : 'py-3'}`}>
          {/* Logo và Tiêu đề */}
